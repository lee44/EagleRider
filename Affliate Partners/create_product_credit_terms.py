...
def create_product_credit_terms(page):
    ...
    for i in range(3):
        page.goto(
            "https://www.eaglerider.com/activeadmin/company_product_credit_terms/new")
        page.fill("input[name=\"company_product_credit_term[company_product_id]\"]", str(company_product_id))

        if i != 1:
            page.fill(
                "input[name=\"company_product_credit_term[due_days]\"]", "30")
        else:
            page.fill(
                "input[name=\"company_product_credit_term[due_days]\"]", "46")
